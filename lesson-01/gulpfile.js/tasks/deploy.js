...
async function getBranch() {
  return new Promise((resolve, reject) => {
    return child_process.exec('git rev-parse --abbrev-ref HEAD', (error, stdout) => {
        if (error)
          reject(`getBranch Error: ${error}`);
...
async function deployCheck(cb) {
  let branch = await getBranch();
  if (settings.environment == 'production' && branch != 'master') {
    cb(new Error('You need to check out the master branch to publish to production bucket'));
  } else {
    cb();
  }
}
...
exports.deployCheck = deployCheck;
